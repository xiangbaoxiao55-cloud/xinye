...
> ⛔ **本文是"手机时代"（2026-05～07）的设计文档，2026-09-16 改版后已经和现状对不上了。**
> 现在它是主 APP 底部「碎碎念」Tab 里的一页信息流：没有锁屏/桌面/壁纸/APP 图标，
> 备忘录笔记 + 草稿箱 + 心情 合成一条流，歌单/你说的话/浏览/待办 是标签，相册是大图卡片。
> 当前实现以 `phone.html` 本体为准；背景和踩坑见 `~/.claude/projects/.../memory/project_phone.md`。
> **下面留着的价值只有一个**：第一节那份 `phone_state` 指令的完整旧文本，备查。
