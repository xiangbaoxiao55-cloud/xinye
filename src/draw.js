...
const INSPIRE_SURFACE_CATS=['材质与表面质感','建筑、空间与场景气质','平面设计、印刷与海报亚种'];
...
async function _pickInspireStyleCombo(){
  ...
  const surfacePool=all.filter(s=>INSPIRE_SURFACE_CATS.includes(s['类别']));
  if(!basePool.length) return null;
  const base=basePool[Math.floor(Math.random()*basePool.length)];
  const combo=[base];
  if(surfacePool.length&&Math.random()<0.6){
    const surf=surfacePool[Math.floor(Math.random()*surfacePool.length)];
    combo.push(surf);
  }
  const tokens=combo.map(s=>s['English prompt tokens']).filter(Boolean).join(', ');
  const names=combo.map(s=>s['中文风格名']).filter(Boolean).join(' × ');
  const rescue=combo.map(s=>s['补救提示']).filter(Boolean).join('；');
  return {tokens,names,rescue};
}
...
function _buildInspireFormat(styleCombo, highRatedExamples){
  const styleHint=styleCombo
    ?`\n\n🎨 本次指定风格锚点：${styleCombo.names}\nPrompt中必须以 "${styleCombo.tokens}" 开头，并围绕这个风格展开。${styleCombo.rescue?`注意：${styleCombo.rescue}`:'对于情侣人物画面，加上 recognizable subjects, expressive faces。'}`
    :'';
  const examplesHint=highRatedExamples
    ?`\n\n⭐ 用户验证过的好prompt示范（学习用词密度和细节颗粒度，不要抄场景）：\n${highRatedExamples}\n---`
