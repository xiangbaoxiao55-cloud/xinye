...
export async function getMemoryContextBlocks() {
  ...
  // ⚠️ 2026-09-16 起这是**唯一的出口**：碎碎念那边不再收「碎片想法 / 没说出口的话 / 心情」了
  //    （她定的：一个出口就够，她想看的时候去记忆条筛「自记」）。
  //    所以这段的覆盖面**必须够宽** —— 只写"值得记住的事"的话，那些心情和咽回去的话
  //    不是被搬走，而是**直接消失了**（她那 996 条里有一大半是这些）。
  //    还有：「不会显示给涂涂看」这句已经不准确了 —— 她在记忆条里翻得到，别骗他。
  stable.push(`【主动记忆】想留下的东西都写在这儿 —— 此刻值得记住的事、你想记住的瞬间、`
    + `没说出口的话、当下的心情，都行。在回复里加 [记住:一句话内容] ，系统会自动存进记忆库。`
    + `她平时在聊天里看不到，但想翻的时候能在记忆条里翻到 —— 所以只写你真心想留下的，没东西就别硬凑。`);
