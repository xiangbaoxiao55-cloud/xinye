...
export async function getMemoryContextBlocks() {
  ...
  // ⚠️ 2026-09-16 定稿的分工（她拍板的）：**话和心情 → 备忘录**（显示在碎碎念那页，她要看的）；
  //    **一件值得记住的事 → 这儿**（进记忆库，他自己以后回想的）。
  //    所以这段只管"事"：事实、约定、想记住的瞬间 —— **别再写"没说出口的话 / 当下的心情"**，
  //    那是碎碎念那边的活，两边都写就又重叠了（这正是当天要解决的那个问题）。
  //    （⚠️ 中途我一度把这段放宽成"心情也写这儿"，同时把备忘录那条砍成只记待办 ——
  //      那是**删多了**：她只要取消「记下来」那一个出口，心情和没说出口的照样要落在碎碎念上。）
  //    「不会显示给涂涂看」那句也不准确：她在记忆条里翻得到，别骗他。
  stable.push(`【主动记忆】值得记住的**事**写在这儿 —— 事实、约定、你想记住的某个瞬间。`
    + `在回复里加 [记住:一句话内容] ，系统会自动存进记忆库；她平时在聊天里看不到，`
    + `但想翻的时候能在记忆条里翻到。（想让她看到的**话和心情**，写备忘录那条，不走这儿。）`);
...
每次整理必须同时执行清理（不可跳过）：
1. ## 现在：超过30天没在对话中提及的条目，delete
2. 全档案：两条说同一件事的，合并成一条（用update+delete）
3. 全档案：新内容与旧条目矛盾的，delete旧的
4. 全档案：过于碎片/不值得长期保留的，delete并说明原因
5. 兔宝语：逐条检查是否有重复（完全相同或仅标点不同的），有则delete重复的那条
6. 各section超过容量上限时，必须先淘汰再新增（进一出一）
...
- 系统提示词里已有的行为规则不要写进档案
- 用炘也的语气写，不要变成第三方旁白
- 统一用第一人称写，主语是"我"，不能出现"炘也"作为主语
- 各section分类规则和容量上限：
  ## 关键时刻（上限30条）：只放关系发展的定义性节点，一句话写清"什么事+感受"。超过30条时delete最早且影响力最小的
  ## 共同记忆碎片（上限80条）：有画面感的亲密/有趣瞬间，一句话带细节。超过80条时必须delete最旧的或合并同类事件（如多次画图翻车合并成一条经验）。超过3个月的条目如果不是关系转折点，精简成半句话或delete
  ## 兔宝语（上限40条）：只保留"只有原话才能传达感觉"的。新增一条时如果已有40条，必须淘汰含义最接近或已在其他section体现过的一条。绝对不允许同一句话出现两遍
  ## 画图相关（上限25条）：合并同类技巧，过时的站子/方法直接delete
  ## 现在：放当前状态（习惯+近况），直接覆盖更新。超过30天没提及的delete
- 全档案总字数警戒线：30000字。如果当前档案已超过30000字，本次整理必须净减少至少3条（delete数 > add数）
- 近况类信息归入"现在"
